...
| Live 120 Hz ZC was intermittently classified stale | `micros()` was captured before the ISR snapshot, allowing an intervening edge to underflow unsigned pulse age; slow status responses could then exceed the 1.5 s AC threshold | Timestamp after snapshot, detect real AC sessions from ISR-measured gaps >=250 ms, preserve mode through shorter gaps, and split large histories from periodic status | Installed schedule remained driving under concurrent client load with stable ZC and no additional AC transition |
